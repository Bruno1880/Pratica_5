...
Foram desenvolvidos scripts (em Shell Script e Python) e criou-se "Unit Files" (arquivos `.service`). Esses arquivos de configuração permitiram que nossos programas fossem executados automaticamente assim que a placa é ligada, sem necessidade de intervenção manual ou login de usuário, utilizando o comando `systemctl` para iniciar, parar, habilitar e verificar o status desses serviços em tempo real.
